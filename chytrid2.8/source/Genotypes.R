Genotypes <- function(susceptibilities, n.loci, n.alleles) {
  n.gtypes <- length(unique(susceptibilities[, 1]))
  ...
  OUT <- NULL  
  for (l in 1:n.loci){ 
    sg1=gtypes[sample(1:length(gtypes[,1]), n),]
    OUT <- cbind(OUT, sg1)
  }
  ...
  susceptibilities <- cbind(susceptibilities, output)
}
